...
and returns a (matched: bool, description: str|None, direction: 'UP'|'DOWN'|None)
tuple evaluated on the last candle. `direction` is this rule's read on where
price goes next — used by the rule-based backtest engine (§9 week 3, no AI
involved yet) to decide long vs. short; the live AI decision layer (§6)
ignores it and reasons over the raw trigger list itself.
"""
...
def rsi_cross_up_30(candles):
    ...
    if p["rsi14"] < 30 <= c["rsi14"]:
        return True, f"RSI ตัดขึ้นผ่าน 30 ({p['rsi14']:.1f} → {c['rsi14']:.1f})", "UP"
    return False, None, None
...
def rsi_cross_down_70(candles):
    ...
    if p["rsi14"] >= 70 > c["rsi14"]:
        return True, f"RSI ตัดลงผ่าน 70 ({p['rsi14']:.1f} → {c['rsi14']:.1f})", "DOWN"
    return False, None, None
...
def macd_hist_sign_change(candles):
    ...
    if p["macd_hist"] * c["macd_hist"] < 0:
        direction = "UP" if c["macd_hist"] > 0 else "DOWN"
        return True, f"MACD histogram เปลี่ยนเครื่องหมาย ({p['macd_hist']:.4f} → {c['macd_hist']:.4f})", direction
    return False, None, None
...
def price_cross_ema50(candles):
    ...
    if (p["close"] - p["ema50"]) * (c["close"] - c["ema50"]) < 0:
        direction = "UP" if c["close"] > c["ema50"] else "DOWN"
        return True, f"ราคาตัดผ่าน EMA50 ({'ขึ้น' if direction == 'UP' else 'ลง'})", direction
    return False, None, None
...
def ema20_cross_ema50(candles):
    ...
    if (p["ema20"] - p["ema50"]) * (c["ema20"] - c["ema50"]) < 0:
        direction = "UP" if c["ema20"] > c["ema50"] else "DOWN"
        label = "golden cross" if direction == "UP" else "death cross"
        return True, f"EMA20 ตัด EMA50 ({label})", direction
    return False, None, None
...
def price_cross_ema200(candles):
    ...
    if (p["close"] - p["ema200"]) * (c["close"] - c["ema200"]) < 0:
        direction = "UP" if c["close"] > c["ema200"] else "DOWN"
        return True, f"ราคาตัดผ่าน EMA200 ({'ขึ้น' if direction == 'UP' else 'ลง'})", direction
    return False, None, None
...
def volume_spike(candles):
    ...
    if c["volume"] > 2 * c["vol_ma20"]:
        return True, f"Volume {c['volume'] / c['vol_ma20']:.1f}× MA20", _candle_direction(c)
    return False, None, None
...
def touches_bb_edge(candles):
    ...
    if c["high"] >= c["bb_upper"]:
        return True, "ราคาแตะ BB upper", "DOWN"  # mean-reversion read: overbought
    if c["low"] <= c["bb_lower"]:
        return True, "ราคาแตะ BB lower", "UP"  # mean-reversion read: oversold
    return False, None, None
...
def bb_squeeze_release(candles):
    ...
    if rank < 0.20 and c["bb_width"] > p["bb_width"] * 1.10:
        return True, f"BB squeeze คลายตัว (percentile {rank * 100:.0f}% → ขยาย)", _candle_direction(c)
    return False, None, None
...
def atr_spike(candles):
    ...
    if c["atr14"] > 1.5 * avg:
        return True, f"ATR {c['atr14']:.4g} > 1.5× ค่าเฉลี่ย 20 แท่ง ({avg:.4g})", _candle_direction(c)
    return False, None, None
...
def breakout_20bar(candles):
    ...
    if c["high"] > hi:
        return True, f"ราคาทะลุ high 20 แท่ง ({hi:.4g})", "UP"
    if c["low"] < lo:
        return True, f"ราคาทะลุ low 20 แท่ง ({lo:.4g})", "DOWN"
    return False, None, None
...
def btc_big_move(candles, btc_change_4h=None):
    ...
    if abs(btc_change_4h) > 3:
        direction = "UP" if btc_change_4h > 0 else "DOWN"
        return True, f"BTC เคลื่อนไหว {btc_change_4h:+.1f}% ใน 4 ชม.", direction
    return False, None, None


def evaluate_rules(candles, btc_change_4h=None):
    """candles: ascending list of ohlcv+indicator dicts, most recent last.
    Returns list of {code, weight, group, description, direction} for matched rules."""
    if len(candles) < 2:
        return []
    matched = []
    for r in RULES:
        fn = r["fn"]
        matched_flag, desc, direction = fn(candles, btc_change_4h) if r["code"] == "E3" else fn(candles)
        if matched_flag:
            matched.append({"code": r["code"], "weight": r["weight"], "group": r["group"],
